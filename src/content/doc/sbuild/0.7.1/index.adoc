...
SBuild buildfiles are Scala source files and must be compile before SBuild can use them. 
...
|====
| `@version`
| Contains the minimal SBuild version that is required for this buildfile. 
...
| Additional plugins and libraries that should be used by this buildfile have to be declared here.
  ...
  If the entry is a plugin, SBuild will automatically load it's dependencies.
